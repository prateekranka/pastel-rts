  ...
  for (let i = 0; i < text.length; i += 1) {
    ...
    if (unit >= 0xd800 && unit <= 0xdbff && i + 1 < text.length) {
      ...
      if (extra >= 0xdc00 && extra <= 0xdfff) {
        codePoint = ((unit - 0xd800) << 10) + (extra - 0xdc00) + 0x10000;
        i += 1;
      }
    }
    if (codePoint < 0x80) {
...
/**
 * Browser-safe SHA-256. Content hashes must match Node's `createHash('sha256')`
 * without importing `node:crypto`, which Vite cannot bundle into game-web.
 */
export function sha256Hex(text: string): string {
  const bytes = utf8Bytes(text);
  const lengthWithMarker = bytes.length + 1;
  const zeroPad = (56 - (lengthWithMarker % 64) + 64) % 64;
  const padded = new Uint8Array(lengthWithMarker + zeroPad + 8);
  padded.set(bytes);
