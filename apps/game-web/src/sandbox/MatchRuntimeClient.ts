import { TICK_MS } from '@pastel-rts/simulation';
...
import { INTERACTION_SNAPSHOT_STRIDE } from './snapshot';
import type { LabControlMessage, LabSnapshotSlot, LabWorkerOutbound } from './types';

export type MatchRuntimeClientOptions = {
  maxEntities?: number;
};

/** Interaction-lab worker client — mirrors M0 SimClient interpolation semantics. */
export class MatchRuntimeClient {
  ...
  constructor(
    options: MatchRuntimeClientOptions = {},
  ...
  start(init: Extract<LabControlMessage, { type: 'initLab' }>): void {
    this.stop();
    this.worker = new Worker(new URL('./interactionWorker.ts', import.meta.url), { type: 'module' });
