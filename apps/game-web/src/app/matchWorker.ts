...
function emitChecksums(): void {
  if (!simulation) {
    return;
  }
  const checksums: readonly StateChecksum[] = simulation.getChecksums();
  post({ type: 'checksums', checksums: [...checksums] });
}
...
function tickOnce(ignorePause = false): void {
  if (!simulation) {
    return;
  }
  const wasPaused = simulation.isPaused();
  if (ignorePause && wasPaused) {
    simulation.resume();
  }
...
export type { CommandResult } from '@pastel-rts/content-schema';
