...
for (const id of ['id', 'name', 'faction', 'anchorX', 'anchorY', 'worldHeight', 'radius']) {
  document.querySelector(`#${id}`)?.addEventListener('input', refreshManifest);
}
fileInput?.addEventListener('change', async (event) => {
  ...
  try {
    ...
    pngDataUrl = await fileToPngDataUrl(file);
...
function refreshManifest(): void {
  ...
  try {
    ...
    const pre = document.querySelector('#manifest');
    if (pre) {
...
async function publishUnit(): Promise<void> {
  ...
  try {
    const response = await fetch('/dev-content/units', {
      ...
      body: JSON.stringify({ manifest: currentManifest, pngBase64: pngDataUrl }),
    });
